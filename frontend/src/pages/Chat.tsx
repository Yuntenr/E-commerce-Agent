...
import { cn, summarizeResult } from "../utils/format";
import { streamQuery } from "../utils/agentApi";
import { MessageBubble } from "../components/MessageBubble";
import type { AgentEvent, ChatMessage, StepState } from "../types/agent";

const examples = [
    "统计 2026 年第一季度各大区的 GMV，并按 GMV 从高到低排序",
    "统计 2026 年 1 月各商品品类的销量和销售额",
...
export default function Chat() {
    ...
    const startQuery = async (rawQuery = draft) => {
        ...
        try {
            await streamQuery(query, user.id, sessionId, { signal: controller.signal, onEvent });
            setMessages((current) =>
                current.map((message) =>
                ...
                <aside className=" hidden min-h-0 border-r border-ink/10 bg-[#efe6d8]/85 backdrop-blur lg:flex lg:flex-col">
                    ...
                    <div className=" border-b border-ink/10 px-5 py-5">
                        <div className="flex items-center gap-3">
                            <div className="grid h-10 w-10 place-items-center bg-ink text-paper">
                                <ChartNoAxesCombined className="h-5 w-5" aria-hidden="true" />
                            </div>
                            <div>
                                <div className="text-base font-semibold tracking-[0.02em]">电商问数</div>
                                <div className="text-xs text-ink/50">E-commerce-Agent</div>
                            </div>
                        </div>
                    </div>

                    {/* 添加新会话 一个可以占据剩余空间、内容超出后自动滚动、内部元素垂直间隔固定的容器。*/}
                    <div className=" min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-4">
                        <button
                            type="button"
                            onClick={clearConversation}
                            disabled={false}
                            className="flex h-11 w-full items-center justify-center gap-2 bg-ink text-sm font-semibold text-paper transition hover:bg-soot disabled:cursor-not-allowed disabled:bg-ink/35">
                        ...
                        <section>
                            ...
                            <div className='space-y-2'>
                                ...
                                (<button
                                    ...
                                    className='w-full border border-ink/10 bg-white/42 px-3 py-3 text-left text-sm leading-5 text-ink/75 transition hover:border-b-blue-200/45 hover:bg-white/75 disabled:cursor-not-allowed disabled:opacity-55'>
                                    {example}
                                </button>)
                                )}
                            </div>
                        ...
                        <div className='grid gap-2 text-xs text-ink/55'>
                            <div className='flex items-center justify-between gap-3'>
                                <span className='inline-flex items-center gap-2'>
                                    <Radio className='h-4 w-4' aria-hidden="true" />API
                                </span>
                            ...
                            <div className='flex items-center justify-between'>
                                <span className='inline-flex items-center gap-2'>
                                    <FileChartColumnIncreasing className='h-4 w-4' aria-hidden="true" />完成
                                </span>
                ...
                <main className="flex min-h-0 min-w-0 flex-col overflow-hidden">
                    ...
                    <header className="flex h-16 shrink-0 items-center justify-between border-b border-ink/10 bg-parchment/88 px-4 backdrop-blur lg:px-6">
                        ...
                        <button
                            ...
                            onClick={clearConversation}
                            disabled={false}
                            className={cn("grid h-9 w-9 place-items-center rounded-full text-ink/55 transition hover:bg-ink/5 hover:text-ink disabled:cursor-not-allowed disabled:opacity-35")}
                            title="清空"
                            aria-label="清空"
                        >
                    ...
                    <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                        {messages.length === 0 ? (
                            <WelcomePanel examples={examples} onUseExample={(example) => setDraft(example)} />
                        ) : (
